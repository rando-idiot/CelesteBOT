...
const locales = {};
let template = JSON.parse(fs.readFileSync(`./locales/en-US.json`).toString("utf-8"))
...
for (const f of localeFiles) {
    ...
    const localeString = fs.readFileSync(`./locales/${f}`).toString("utf-8");
    try { JSON.parse(localeString) } catch { log.warn(`File ${f} is not valid JSON`); continue };
    let localeObject = JSON.parse(fs.readFileSync(`./locales/${f}`).toString("utf-8"))
